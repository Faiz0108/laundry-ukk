...
import { FaChartBar, FaMoneyBillWave, FaUsers, FaShoppingCart, FaFilePdf } from "react-icons/fa";
...
export default function ReportsAdmin() {
    ...
    const [statusCounts, setStatusCounts] = useState({ baru: 0, proses: 0, selesai: 0, diambil: 0 });
    ...
    const fetchData = async () => {
        ...
        const { count: txCount } = await supabase.from("tb_transaksi").select("*", { count: "exact", head: true }).gte("tgl", startDate).lte("tgl", endDate);
        setTotalTx(txCount || 0);

        const { count: memCount } = await supabase.from("tb_member").select("*", { count: "exact", head: true }).gte("created_at", startDate).lte("created_at", endDate);
        setTotalMembers(memCount || 0);

        const { data: allTx } = await supabase.from("tb_transaksi").select("id, kode_invoice, tgl, batas_waktu, status, dibayar, tb_member(nama), tb_detail_transaksi(qty, tb_paket(nama_paket, harga))").gte("tgl", startDate).lte("tgl", endDate);
        let rev = 0;
        const sc = { baru: 0, proses: 0, selesai: 0, diambil: 0 };
        allTx?.forEach(tx => {
            if (tx.status && tx.status in sc) sc[tx.status as keyof typeof sc]++;
            let txTotal = 0;
    ...
    const exportPDF = async () => {
        ...
        const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Laporan Admin</title>
        <style>
            ...
            .status-card{flex:1;border:1px solid #e5e7eb;border-radius:8px;padding:12px;text-align:center}
            .status-count{font-size:24px;font-weight:700}
            .status-label{font-size:11px;color:#666}
            .text-right{text-align:right}
        </style></head><body>
        <h1>📊 Laporan Admin - Periode ${monthName} ${selectedYear}</h1>
        ...
        <div class="status-section">
            <h3 style="font-size:14px;margin-bottom:8px">Status Transaksi</h3>
            <div class="status-grid">
                <div class="status-card"><p class="status-count" style="color:#2563eb">${statusCounts.baru}</p><p class="status-label">Baru</p></div>
                <div class="status-card"><p class="status-count" style="color:#d97706">${statusCounts.proses}</p><p class="status-label">Proses</p></div>
                <div class="status-card"><p class="status-count" style="color:#16a34a">${statusCounts.selesai}</p><p class="status-label">Selesai</p></div>
                <div class="status-card"><p class="status-count" style="color:#6b7280">${statusCounts.diambil}</p><p class="status-label">Diambil</p></div>
            </div>
        </div>
        ...
        const opt = {
            margin: 0.5,
            filename: `Laporan_Admin_${monthName}_${selectedYear}.pdf`,
            ...
            <div style={{ background: "linear-gradient(145deg, #ffffff 0%, #f8fafc 100%)", padding: "28px 32px", borderBottom: "1px solid #e2e8f0", position: "relative" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                        <div style={{ width: "56px", height: "56px", background: "linear-gradient(135deg, #22c55e, #16a34a)", borderRadius: "18px", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "28px", boxShadow: "0 10px 20px -5px rgba(34, 197, 94, 0.3)" }}><FaChartBar /></div>
                        <div>
                            <h1 style={{ fontSize: "28px", fontWeight: "700", color: "#0f172a", margin: 0 }}>Laporan Admin</h1>
                            <p style={{ fontSize: "14px", color: "#475569", margin: "4px 0 0 0" }}>Ringkasan laporan seluruh operasional laundry</p>
                        </div>
                    </div>
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "28px 32px", background: "linear-gradient(145deg, #ffffff 0%, #fafcff 100%)" }}>
                {loading ? (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px", background: "white", borderRadius: "24px", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.1)" }}>
                        <div style={{ width: "56px", height: "56px", border: "3px solid #e2e8f0", borderTopColor: "#22c55e", borderRadius: "50%", animation: "spin 1s linear infinite", marginBottom: "20px" }} />
                    ...
                    <>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px", marginBottom: "32px" }}>
                            {[
                                ...
                                { label: "Total Transaksi", value: String(totalTx), icon: FaShoppingCart, color: "#3b82f6", bg: "linear-gradient(135deg, #eff6ff, #dbeafe)" },
                                { label: "Total Pelanggan Baru", value: String(totalMembers), icon: FaUsers, color: "#f97316", bg: "linear-gradient(135deg, #fff7ed, #ffedd5)" },
                            ].map((s, i) => {
                                const Icon = s.icon;
                                return (
                        ...
                        <div style={{ background: "white", borderRadius: "24px", padding: "28px", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 0 0 1px rgba(0, 0, 0, 0.02)", animation: "fadeIn 0.5s ease 0.3s both" }}>
                            <h3 style={{ fontSize: "18px", fontWeight: "600", color: "#0f172a", marginBottom: "20px" }}>📊 Status Transaksi</h3>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px" }}>
                                {[
                                    { label: "Baru", count: statusCounts.baru, color: "#2563eb", bg: "#dbeafe" },
                                    { label: "Proses", count: statusCounts.proses, color: "#d97706", bg: "#fef3c7" },
                                    { label: "Selesai", count: statusCounts.selesai, color: "#16a34a", bg: "#dcfce7" },
                                    { label: "Diambil", count: statusCounts.diambil, color: "#6b7280", bg: "#e5e7eb" },
                                ].map((s, i) => (
                                    <div key={i} style={{ backgroundColor: s.bg, borderRadius: "16px", padding: "20px", textAlign: "center", transition: "transform 0.2s ease" }}
                                        onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.05)"; }}
                                        onMouseLeave={(e) => { e.currentTarget.style.transform = "scale(1)"; }}>
                                        <p style={{ fontSize: "32px", fontWeight: "700", color: s.color, margin: "0 0 4px 0" }}>{s.count}</p>
                                        <p style={{ fontSize: "14px", color: s.color, margin: 0, fontWeight: "500" }}>{s.label}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
